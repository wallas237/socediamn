            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-striped" id="dataTable" width="100%" cellspacing="0">
                        ...
                        <tbody>
                            @php
                                $inscript = DB::table('inscriptions')->where('labo', '>=', 1)->get();
                            @endphp
                            @foreach ($inscript as $v)
                                <tr>
                                    ...
                                    <td>
                                        @php
                                            $grade = DB::table('laboratoires')->where('id', $v->labo)->first();
                                            $specialiste = DB::table('specialites')
                                                ->where('id', $v->specialite)
                                                ->first();
                                        @endphp
                                        {{ !empty($grade) ? $grade->labo : '' }}
                                    </td>
                                    <th>{{ !empty($specialiste) ? $specialiste->speciality : 'Non définie' }}</th>
                                    <td>{{ $v->confirmation_attestion == 1 ? 'Badge Effectué' : 'Non' }}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton7">
                                                ...
                                                <a class="dropdown-item"
                                                    href="/effectuer-badge-labo/{{ $v->labo }}" target="_blank">
                                                    <button class="btn btn-success p-2">
                                                        Effectuer les badges du labo
                                                    </button>
                                                </a>
