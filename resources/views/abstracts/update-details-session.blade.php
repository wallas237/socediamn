        ...
        <div class="card shadow mb-4 w-50">
            <div class="card-header py-3">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <h6 class="m-0 font-weight-bold text-black-50">Modifier une session</h6>

                </div>

            </div>
            @php
                $communicationSalle = DB::table('communication_salles')->get();

                $v = DB::table('ajouter_detail_sessions')->where('id', $idDetails)->first();
            @endphp
            <div class="card-body">
                <section>
                    ...
                    <form method="post" action="{{ route('update.details', ['idDetails' => $idDetails]) }}"
                        ...
                        <div class="pt-3">
                            ...
                            <select name="communication_salle_id" id="communication_salle_id"
                                ...
                                @foreach ($communicationSalle as $salle)
                                    @if ($salle->id == $v->communication_salle_id)
                                        <option value="{{ $salle->id }}" selected>{{ $salle->libelle_session }}
                                        </option>
                                    @else
                                        <option value="{{ $salle->id }}">{{ $salle->libelle_session }}</option>
                                    @endif
                                @endforeach
                            </select>
                        </div>
                        <div class="pt-3">
                            <x-input-label for="libelle_detail_session" class="form-check-label text-dark"
                                :value="old('Titre Détail Session', $v->libelle_detail_session)" />
                            <textarea name="libelle_detail_session" id="" cols="40" rows="15"
                                class="form-control bg-white block text-dark">{{ old('libelle_detail_session', $v->libelle_detail_session) }}</textarea>
                        ...
                        <div class="pt-3">
                            ...
                            <x-text-input id="orateur" name="orateur" type="text"
                                class="form-control bg-white block text-dark" :value="old('orateur', $v->orateur)" autofocus
                        ...
                        <div class="pt-3">
                            ...
                            <x-text-input id="libelle_salle" name="libelle_salle" type="text"
                                class="form-control bg-white block text-dark" :value="old('libelle_salle', $v->libelle_salle)" autofocus
                        ...
                        <tbody>
                            ...
                            @php
                                $comSalle = DB::table('communication_salles')
                                    ->where('id', $v->communication_salle_id)
                                    ->first();
                            @endphp
                            <tr>
                                <td>
                                    <div class="dropdown">
                                        <button class="btn btn-info p-2 dropdown-toggle" id="dropdownMenuIconButton7"
