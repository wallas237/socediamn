    ...
    <div class="container">
        @foreach ($data['participants'] as $participant)
            @php
                ...
                $grade = !empty($grades) ? $grades->titre : "Visteur";
                $libelle = "PARTICIPANT";
            ...
            @if ($i == 1)
                <div style="margin-top: {{ '-6%;' }} position: absolute; margin-left: {{ '-5.5%' }}"
                    ...
                    <h1 style="top: 57%;">{{ $name }}</h1>
                     <p style="top: 62%; font-size: 1.5em; color: whitesmoke;">{{ $grade }}</p>
                    {{--  <h2 style="top: 53%;">{{ $ville }}</h2>  --}}
                     <h2 style="top: 79%; position: absolute; font-weight: bolder; color:aliceblue; font-size: 1.7em;" >{{ $libelle }} </h2>
                    <div class="qr-code" style="top: 05%; left: 3%;">
            ...
            @elseif($i == 2)
                <div style=" position: absolute; top: -4%; right: -5.5%;" class="{{ $i }}">
                    ...
                    <p style="top: 59%; font-size: 1.5em; color: whitesmoke;">{{ $grade }}</p>
                    {{--  <h2 style="top: 49%;">{{ $ville }}</h2>  --}}
                    <h2 style="top: 79%; position: absolute; font-weight: bolder; color:aliceblue; font-size: 1.7em;" >{{ $libelle }} </h2>
                    <div class="qr-code" style="top: 01%; left: 3%;">
                        <img src="{{ $lienImage }}" alt="" />
                    </div>
                </div>
                @php
            ...
            @elseif($i == 4)
                <div style="position: absolute; top: 50%;  left: {{ '-5.5%' }}" class="{{ $i }}">
                    ...
                    <p style="top: 59%; font-size: 1.5em; color: whitesmoke;">{{ $grade }}</p>
                    {{--  <h2 style="top: 49%;">{{ $ville }}</h2>  --}}
                    <h2 style="top: 60%; position: absolute; font-weight: bolder; color:aliceblue; font-size: 1.7em;" >{{ $libelle }} </h2>
                    <div class="qr-code" style="top: 01%; left: 3%;">
                        <img src="{{ $lienImage }}" alt="" />
                    </div>
